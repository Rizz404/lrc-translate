...
func TestTranslate_SucceedsFirstTry(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(chatResponse("halo dunia", "stop"))
	}))
	defer srv.Close()
...
func TestTranslate_StripsThinkBlockAndQuotes(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(chatResponse("<think>hmm let's see...</think>\"halo dunia\"", "stop"))
	}))
	defer srv.Close()
...
func TestTranslate_SendsBearerTokenWhenAPIKeySet(t *testing.T) {
	var gotAuth string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotAuth = r.Header.Get("Authorization")
		json.NewEncoder(w).Encode(chatResponse("ok", "stop"))
	}))
	defer srv.Close()

	c := New(srv.URL, "sk-secret", "test-model")
	if _, err := c.Translate(context.Background(), "hi", "en", "id"); err != nil {
		t.Fatalf("Translate returned error: %v", err)
	}
	if gotAuth != "Bearer sk-secret" {
		t.Errorf("got Authorization %q, want %q", gotAuth, "Bearer sk-secret")
	}
}

func TestTranslate_OmitsAuthHeaderWhenAPIKeyEmpty(t *testing.T) {
	var gotAuth string
	sawHeader := false
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotAuth = r.Header.Get("Authorization")
...
func TestTranslate_RetriesOnEmptyAnswerTruncatedByLength(t *testing.T) {
	var attempts int32
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if atomic.AddInt32(&attempts, 1) < 3 {
			json.NewEncoder(w).Encode(chatResponse("", "length"))
			return
		}
		json.NewEncoder(w).Encode(chatResponse("ok", "stop"))
	}))
